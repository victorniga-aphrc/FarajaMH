...
def _user_payload(u: User):
    return {
        ...
        "reset_password": getattr(u, "reset_password", False),
    }
...
def login():
    ...
    try:
        ...
        # Clear previous user's conversation so this user gets a fresh one (avoid mix-up)
        for key in ("conversation_id", "id", "conv"):
            session.pop(key, None)
        return jsonify({"ok": True, "user": _user_payload(u)})
    finally:
        db.close()
