...
## 📱 Using the app (chat modes)

### Modes

- **Real Actors** — Turn-based: you play both sides. Choose **Clinician** or **Patient**, type (or use Voice), then **Send**. Switch role and send again for the next turn. Intended for training or demos where an admin acts as patient and clinician.
- **Simulated** — Single “patient” speaker; the AI plays clinician and recommender. No role toggle.
- **Live (Mic)** — Continuous speech: click **Start Live**, speak; text appears as you talk. Click **Stop** to flush and get final transcript. No manual turn switching.

### In Real Actors / Simulated: Send vs Voice

- **Send** — Sends whatever is in the **Message** box as one turn, using the currently selected role (Clinician or Patient). Type (or paste) text and click Send.
- **Voice** — **Click once** to start recording; **click again** to stop. The app transcribes the recording (batch), puts the text in the Message box, then **automatically submits** it as one turn with the current role. So: select role → Voice → speak → stop → transcribed text is sent as that role.

So **Send** = “send the typed message”; **Voice** = “record → transcribe → send that as the current role.”
